...
void main() {
	...
	sum += val;
	val *= texture2D(mainTexture, texCoord1.xy).xyz;
	gl_FragColor.xyz = val;

	val = vec3(0.0, 0.5, 1.0);
	sum += val;
	val *= texture2D(mainTexture, texCoord1.zw).xyz;
	gl_FragColor.xyz += val;

	val = vec3(0.0, 1.0, 0.0);
	sum += val;
	val *= texture2D(mainTexture, texCoord2.xy).xyz;
	gl_FragColor.xyz += val;

	val = vec3(1.0, 0.5, 0.0);
	sum += val;
	val *= texture2D(mainTexture, texCoord2.zw).xyz;
	gl_FragColor.xyz += val;

	val = vec3(1.0, 0.4, 0.0);
	sum += val;
	val *= texture2D(mainTexture, texCoord3.xy).xyz;
	gl_FragColor.xyz += val;

	val = vec3(1.0, 0.3, 1.0);
	sum += val;
	val *= texture2D(mainTexture, texCoord3.zw).xyz;
	gl_FragColor.xyz += val;
