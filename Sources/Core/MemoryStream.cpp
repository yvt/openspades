...
namespace spades {
	MemoryStream::MemoryStream(char *buffer,
							   size_t len,
							   bool allowWrite) {
	...
	MemoryStream::MemoryStream(const char *buffer,
							   size_t len) {
		SPADES_MARK_FUNCTION();
	...
	size_t MemoryStream::Read(void *data,
							  size_t bytes) {
		SPADES_MARK_FUNCTION();
		if(position >= length){
			return 0;
		...
		memcpy(data, memory + (size_t)position,
			   (size_t)bytes);
	...
	void MemoryStream::SetPosition(uint64_t pos){
		position = pos;
	}
