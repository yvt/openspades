...
namespace spades { namespace ngclient {
	...
	void FTFontSet::AddFace(const std::string &fileName) {
		...
		(GetFreeType(),
		 ...
		 &face);
		if (ret) {
			SPRaise("Failed to load font %s: FreeType error %d",
					fileName.c_str(), ret);
		}
	...
	struct BinPlaceResult {
		client::IImage& image;
		int x, y;
		BinPlaceResult(client::IImage& image, int x, int y):
		image(image), x(x), y(y) { }
	};
	...
	struct FTFont::Bin {
		...
		Place(Bitmap& bmp) {
			...
			while (it2 != skyline.end() &&
				   it != skyline.end()) {
				while (it2 != skyline.end() &&
					   it2->first < it->first + bw) {
					++it2;
				}
				if (it2 == skyline.end()) {
					break;
				...
				(it, it2, [](const std::pair<int, int>& a,
				...
				if (maxY + bh < height) {
					...
					for (auto it1 = it; it1 != it2;) {
						...
						wasted += std::min(segRight, right) *
						(maxY - cur->second);
					}
					
					if (wasted < bestWasted) {
						bestIt1 = it; bestIt2 = it2;
						bestWasted = wasted;
						bestMaxY = maxY;
					}
	...
	FTFont::FTFont(client::IRenderer *renderer,
				   FTFontSet *fontSet,
				   float height,
				   float lineHeight):
	...
	FTFont::Glyph *FTFont::GetGlyph(uint32_t code) {
		...
		for (const auto& face: fontSet->faces) {
			...
			if (cId != 0) {
				auto it2 = glyphs.find
				(std::make_pair<FT_Face>(*face, cId));
				if (it2 == glyphs.end()) {
	...
	void FTFont::SplitTextIntoGlyphs(const std::string &str,
									 T onGlyph,
									 T3 onFallback,
									 T2 onLineBreak) {
		// FIXME: ligatures?
		...
		while (i < str.size()) {
			std::size_t advance;
			auto code = GetCodePointFromUTF8String(str, i,
												   &advance);
			i += advance;
			if (code == 10 || code == 13) {
				if (breakCode == 0) breakCode = code;
				if (code == breakCode) {
					onLineBreak();
				}
				continue;
	...
	Vector2 FTFont::Measure(const std::string& str) {
		...
		SplitTextIntoGlyphs(str, [&](Glyph& g) {
			x += g.advance.x;
	...
	void FTFont::RenderGlyph(Glyph& g) {
		...
		FT_Set_Char_Size(g.face, 0,
						 static_cast<FT_F26Dot6>(height * 64.f),
						 72, 72);
		FT_Render_Glyph(g.face->glyph,
						FT_RENDER_MODE_NORMAL);
		
		auto& bmp = g.face->glyph->bitmap;
		FTBitmapWrapper outbmp;
		FT_Bitmap_Convert(GetFreeType(), &bmp,
						  outbmp, 1);
		
		SPAssert(outbmp->pixel_mode == FT_PIXEL_MODE_GRAY);
		
		Handle<Bitmap> spbmp
		(new Bitmap(outbmp->width + 1, outbmp->rows + 1), false);
		
		memset(spbmp->GetPixels(), 0,
			   4 * spbmp->GetWidth() * spbmp->GetHeight());
		
		for (int y = 0; y < outbmp->rows; ++y) {
			const auto *inpixs = outbmp->buffer +
			y * outbmp->width;
		...
		AABB2 bounds((*result).x, (*result).y,
					 spbmp->GetWidth() - 1,
					 spbmp->GetHeight() - 1);
	...
	void FTFont::RenderBlurGlyph(Glyph& g) {
		RenderGlyph(g);
		if (g.blurImage) return;
		
		enum { KernelSize = 6 };
		
		auto& orig = *g.bmp;
		Handle<Bitmap> newbmp(new Bitmap(orig.GetWidth() + KernelSize,
										 orig.GetHeight() + KernelSize),
							  false);
		...
		Vector2 offs = (*g.image).offset -
			Vector2(1,1) * (KernelSize*0.5f);
		
		g.blurImage.reset((*result).image, bounds, offs);
		
	}
	
	void FTFont::Draw(const std::string& str,
	...
	void FTFont::DrawBlurred(const std::string& str,
					  Vector2 offset, float scale,
					  Vector4 color) {
		SPADES_MARK_FUNCTION();
	...
	void FTFont::DrawShadow(const std::string &text,
