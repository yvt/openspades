...
namespace spades { namespace ngclient {
	...
	class FTFont: public client::IFont {
		...
		struct GlyphHash {
			std::size_t operator () (const std::pair<FT_Face, uint32_t>& p) const {
				return std::hash<FT_Face>()(p.first) ^
				std::hash<uint32_t>()(p.second);
			}
		};
		std::unordered_map<std::pair<FT_Face, uint32_t>, Glyph,
		GlyphHash> glyphs;
		std::unordered_map<uint32_t, std::reference_wrapper<Glyph>>
		glyphMap;
		float lineHeight;
		/** em height */
		float height;
		...
		void SplitTextIntoGlyphs(const std::string&,
								 T glyphHandler,
								 T3 fallbackHandler,
								 T2 lineBreakHandler);
	...
	public:
		...
		void Draw(const std::string&,
				  ...
				  Vector4 color) override;
		void DrawBlurred(const std::string&,
				  Vector2 offset, float scale,
						 Vector4 color);
		void DrawShadow(const std::string&,
				  const Vector2& offset, float scale,
				  const Vector4& color,
				  const Vector4& shadowColor) override;
	};
