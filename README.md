...
1. Install dependencies:
   ...
   *On Debian-derived distributions*: 
   ```
   sudo apt-get install pkg-config libglew-dev libcurl3-openssl-dev libsdl2-dev libsdl2-image-dev libalut-dev xdg-utils
   ```
...
9. To get audio working, download a [Windows release of OpenSpades](https://github.com/yvt/openspades/releases), extract it, and copy to the build output directory:
 * OpenAL: `openal32.dll` 
 * YSR: `YSRSpades.dll` and `libgcc_s_dw2-1.dll`
10. Build the paks using `mkpak.ps1` in PowerShell 5.0 and copy the generated paks into a new `Resources` folder, inside the build output directory. If the script is not working you can also:
 * Copy the `E:/Projects/openspades/Resources/` folder to the build directory instead
